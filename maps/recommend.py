...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    longs = [restaurant_location(rst)[0] for rst in cluster]
    lats = [restaurant_location(rst)[1] for rst in cluster]

    return [mean(longs), mean(lats)]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    targets = [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]

    return max(targets, key = lambda target: target[1])[0]
    # END Question 8
